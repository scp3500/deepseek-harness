      ...
      async selectModel(request) {
        ...
        return serializeImageAdmission(found.agent, async () => {
          try {
            ...
            // Text-only adapters strip image blocks silently, so switching to a
            // non-vision model stays safe even when the session log already holds
            // images (the model inspects them via the view_image bridge instead).
